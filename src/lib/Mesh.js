...
import LightProps from './light.js';
...
export default class Mesh 
{
    constructor(gl, MeshOBJect, proj, color, camera, scaling, ID)
    {
        ...
        {
            console.log("Buffer for Mesh's vertices could Not be allocated");
        }
        
        if(!this.vertexNormalBuffer)
        {
            console.log("Buffer for Mesh's vertexNormals could Not be allocated");
        }	
        ...
        this.lightProps = new LightProps();

        let [maxX, minX, maxY, minY, maxZ, minZ] = this.BBox();
        
        let tempPos = vec4.fromValues(1.15*maxX, 1.15*maxY, 1.15*maxZ,1);
    ...
    {
        let [maxX,minX,maxY,minY,maxZ,minZ] = this.BBox();
        let MaxDistancefromSurface = 0;
        let LightPosition = vec3.fromValues(LightPos[0], LightPos[1], LightPos[2]);
        for(let i=0;i<this.vertexAttributesData.length/3;i+=1)
        {
            let tempPosition = vec4.fromValues(this.vertexAttributesData[i], this.vertexAttributesData[i+1], this.vertexAttributesData[i+2],1)
    ...
    translateLight(position)
    {
        ...
        let tempMax = vec4.fromValues(maxX, maxY, maxZ, 1);
        let tempMin = vec4.fromValues(minX, minY, minZ, 1);
    ...
    setLightAttrs(lightAttr)
    {
        this.lightProps.setAmbient(lightAttr.Ambientcolor);
        this.lightProps.setDiffuse(lightAttr.DiffuseColor);
        this.lightProps.setSpecular(lightAttr.SpecularColor);
        
        this.lightProps.setKa(lightAttr.ka);
        this.lightProps.setKd(lightAttr.kd);
        this.lightProps.setKs(lightAttr.ks);
        
        this.lightProps.setShine(lightAttr.Shine);
        this.lightProps.setLimit(lightAttr.Limit);
    }

    draw(shader)
    {
        // Setting All the Variables
        const modelUniform = shader.uniform("modelMat");
        const projectUniform = shader.uniform("projMat");
        const viewUniform = shader.uniform("viewMat");
        const aPosition = shader.attribute("aPosition");
        const normalLocation = shader.attribute("normalAttr");
        const transposeWorld = shader.uniform("transposeWMat");
        const worldLocation = shader.uniform("worldMat");
        ...
        const viewWorldPosition = shader.uniform("uViewWorldPos");
        const Shininess = shader.uniform("uShinyAmt");
        const color_set = shader.uniform("aColor");
        this.gl.uniform4fv(color_set, this.color);
        // Setting the numerator for attenuation
        // Object Properties
        let shininessVal = this.lightProps.getShine();
        let Ka = this.lightProps.getKa();
        let Kd = this.lightProps.getKd();
        let Ks = this.lightProps.getKs();
        ...
        {
            let LightWorldLocation = shader.uniform("LightPositions[" + i + "].uLightWorldPosition");
            let lightDirectionLocation = shader.uniform("LightPositions[" + i + "].uLightDirection");
            let Ambient = shader.uniform("LightPositions[" + i + "].AmbientColor");
            let Diffuse = shader.uniform("LightPositions[" + i + "].DiffuseColor");
            let Specular = shader.uniform("LightPositions[" + i + "].SpecularColor");
            let maxDist = shader.uniform("LightPositions[" + i + "].maxDist");
            let LightSourceOn = shader.uniform("LightPositions[" + i + "].LightSourceOn");
            ...
            let MaxDistance = this.getMaxDistanceFromSurface(this.LightSources[i].LightPos);
            
            shader.setUniform1f(maxDist, MaxDistance);
            shader.setUniform3fv(LightWorldLocation, this.LightSources[i].LightPos);
            shader.setUniform1i(LightSourceOn, this.LightSources[i].LightSourceOn);
        }
        ...
        // The three types of primitives we can have are: triangles, points and lines
        // But I've generalized it to triangles here because we're only using triangles right now
        this.gl.drawElements(this.gl.TRIANGLES, this.vertexIndices.length, this.gl.UNSIGNED_SHORT, indexBuffer); 
        // this.gl.drawElements(this.gl.LINES, mesh.indexBuffer.numItems, this.gl.UNSIGNED_SHORT, 0);
        // this.gl.drawElements(this.gl.POINTS, mesh.indexBuffer.numItems, this.gl.UNSIGNED_SHORT, 0);
    }
    ...
    updateCamera(cam)
    {
        this.eye = vec3.fromValues(cam.eye.x, cam.eye.y, cam.eye.z);
        this.center = vec3.fromValues(cam.center.x, cam.center.y, cam.center.z);
        this.up = vec3.fromValues(cam.up.x, cam.up.y, cam.up.z);
    }
    ...
    BBox()
    {
        ...
        for(let i=0; i < this.vertexAttributesData.length; i+=1)
        {
            if(i%3 == 0)
            {
